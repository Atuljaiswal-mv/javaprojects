...
if(title==null) throw new DAOException("designation null");

title=title.trim();
if(title.length()==0) throw new DAOException("length of designation is zero");

// now title exists and it is valid by length
...
if(randomAccessFile.length()==0) //if true:- means empty file , so write header :2 strings
{
lastGeneratedCodeString="0";
while(lastGeneratedCodeString.length()<10) lastGeneratedCodeString+=" "; // add space
...
// now pointer is loceted after header: 2 strings
// pointer is pointing to first record
// check duplicacy
int fCode=0;
String fTitle="";
while(randomAccessFile.getFilePointer()<randomAccessFile.length())
{
fCode=Integer.parseInt(randomAccessFile.readLine()); // not necessary
...
// title not exists, pointer is at the end of file, add data
...
int fCode;
String fTitle;
randomAccessFile.readLine();// for lastGeneratedCode
randomAccessFile.readLine();// for recordCount
boolean found=false;
while(randomAccessFile.getFilePointer()<randomAccessFile.length())
{
...
randomAccessFile.readLine();// for title(designation)
}
if(found==false)
{
randomAccessFile.close();
...
// check if the same designation given by user exists with another code
randomAccessFile.seek(0);
...
{
fCode=Integer.parseInt(randomAccessFile.readLine());
fTitle=randomAccessFile.readLine();// for title(designation)
...
randomAccessFile.seek(0);
randomAccessFile.readLine();
int recordCount=Integer.parseInt(randomAccessFile.readLine().trim())-1;
...
{
File file=new File(FILE_NAME);
if(!file.exists()) return designations;// returning Set of size :0 - no records
RandomAccessFile randomAccessFile =new RandomAccessFile(file,"rw");
if(randomAccessFile.length()==0)
{
randomAccessFile.close();
...
{
randomAccessFile.close();
throw new DAOException("Invlaid code :"+code);
}
...
while(randomAccessFile.getFilePointer()<randomAccessFile.length())
{
randomAccessFile.readLine();//for code
fTitle=randomAccessFile.readLine();
if(fTitle.equalsIgnoreCase(title))
{
randomAccessFile.close();
